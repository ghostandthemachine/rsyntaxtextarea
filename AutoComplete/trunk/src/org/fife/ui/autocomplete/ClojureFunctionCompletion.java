 ...
 * 12/22/2008
 *
 * ClojureFunctionCompletion.java - A completion representing a function.
 * 
 ...
 * @version 1.0
 */
public class ClojureFunctionCompletion extends VariableCompletion
	 ...
	 */
	public ClojureFunctionCompletion(CompletionProvider provider, String name,
								String returnType) {
	...
	protected void addDefinitionString(StringBuffer sb) {
		sb.append("<html><b>");
		// semi hack to add clojure opening paren in desc string
		sb.append("(");
	...
	public String getDefinitionString() {
		...
		ClojureCompletionProvider cljProvider = ((ClojureCompletionProvider) provider);
		if(cljProvider.hasSpace()) {
			cljProvider.setCutForSpace(true);
		}
	...
	public String getReplacementText() {
		String originalReplacementText = super.getReplacementText();
		ClojureCompletionProvider cljProvider = ((ClojureCompletionProvider) getProvider());
		String returnText = originalReplacementText;
		if(cljProvider.cutForSpace() && insertingName){
			returnText = originalReplacementText.substring(1);
			// clean up
			cljProvider.setCutForSpace(false);
			insertingName = false;
		}
		return returnText;
	}
	...
	public String getInputText() {
		return ((ClojureCompletionProvider) getProvider()).hasSpace() ? getReplacementText() : getReplacementText() + " ";
	}
