...
public class ClojureCompletionProvider extends AbstractCompletionProvider {
	 ...
	 * @see BasicCompletion
	 */
	public ClojureCompletionProvider(String[] words) {
		init();
	...
	public String getAlreadyEnteredText(JTextComponent comp) {
		...
		start = segEnd - 1;
		hasSpace = false;
		while (start>=seg.offset && isValidPreviousChar(seg.array[start - 1], seg.array[start])) {
			start--;
			hasSpace = true;
			len = dot-start;
		}
	...
	public List getCompletionsAt(JTextComponent tc, Point p) {
		...
		try {
			...
			while ((startOffs>=s.offset && isValidChar(s.array[startOffs])) 
				|| (startOffs>=s.offset && isValidChar(s.array[startOffs - 1]))) {
				startOffs--;
			}
	...
	protected boolean isValidPreviousChar(char prev, char ch) {
		return Character.isLetterOrDigit(prev) && Character.isWhitespace(ch);
	}
	...
	public void setCutForSpace(boolean b) {
		cutForSpace = b;
	}

	public boolean cutForSpace() {
		return cutForSpace;
	}
